...
ENV DEFAULT_PACKAGES="jupyter rasterio scikit-image gdal pandas"
RUN /anaconda/bin/conda create  -y -n gbdx_py3 python=3.7  && \
    ...
    /anaconda/bin/conda install -y -n gbdx_py3 -c digitalglobe gbdxtools=0.17.0

RUN /anaconda/bin/conda create  -y -n gbdx_py2 python=2.7 && \
    /anaconda/bin/conda install -y -n gbdx_py2 scipy=1.0.0 dask=1.1.0 setuptools=44.0.0 && \
